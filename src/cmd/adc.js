...
this.botStart = async function ({ api, event, input }) {
    ...
    if(!text && name) {
        var data = fs.readFile(
          ...
          "utf-8",
          async (err, data) => {
            if (err) return api.sendMessage(`Command ${input[0]} does not exist!.`, threadID, messageID);
            ...
            var link = await pastepin(input[1] || 'noname')
    ...
    if (url[0].indexOf('pastebin') !== -1) {
        axios.get(url[0]).then(i => {
            ...
            fs.writeFile(
                ...
                "utf-8",
                function (err) {
                    if (err) return api.sendMessage(`An error occurred while applying the code ${input[0]}.js`, threadID, messageID);
                    api.sendMessage(`Applied the code to ${input[0]}.js`, threadID, messageID);
                }
            );
        })
    }
